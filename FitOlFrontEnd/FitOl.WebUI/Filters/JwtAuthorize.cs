...
namespace FitOl.WebUI.Filters
{
    public class JwtAuthorize : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            {
                context.Result = new RedirectToActionResult("Calculator", "Home", new { @area = "" });          
            }
            else
            {
                ...
                var responseMessage = httpClient.GetAsync($"http://localhost:62188/api/Auth/ActiveUser/{userName}").Result;

                if (responseMessage.IsSuccessStatusCode)
                {
                    var activeUser = JsonConvert.DeserializeObject<AppUserViewModel>(responseMessage.Content.ReadAsStringAsync().Result);

                    // CustomSessionExtension içersinde biz yazdık genişlettik
